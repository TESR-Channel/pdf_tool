...
const TD = { // tool defaults
  ...
  rubber: { ink: '#c8102e', shape: 'logo', t1: 'TESR CO., LTD.', t2: 'THAI EMBEDDED SYSTEM AND ROBOTICS', mid: '', date: false, asset: null },
...
const ALLTOOLS = [
  ...
  ['ลงนาม', [['t', 'sign', 'ลายเซ็น', 'วาดหรืออัปโหลด', '#8b0000'], ['t', 'stamp', 'ตราประทับ', 'ข้อความ หรือ ตรายาง TESR', '#b8102a'], ['a', 'date', 'ใส่วันที่', 'วันที่วันนี้ภาษาไทย', '#5846d6'], ['t', 'wm', 'ลายน้ำ', 'ข้อความ / โลโก้ TESR', '#b11dab']]],
...
stage.addEventListener('pointerdown', e => {
  ...
  if (tool === 'stamp' && TD.stamp.mode === 'rubber') { if (!TD.rubber.asset) { buildRubberAsset(); return; } const as = assets[TD.rubber.asset]; const w = W * .28, h = w * as.h / as.w; snapshot(); const a = { id: uid(), type: 'image', x: x - w / 2, y: y - h / 2, w, h, asset: TD.rubber.asset, opacity: .92 }; p.annots.push(a); setTool('select'); sel = { page: i, id: a.id }; drawOverlay(i); renderProps(); return; }
...
function renderProps(focusText = false) {
  ...
  if (a) {
    ...
    switch (tool) {
      ...
      case 'stamp': { const r = TD.rubber; html = head('ตราประทับ') + `<div class="opt">${F.seg('stMode', 'ชนิด', [['text', 'ข้อความ'], ['rubber', 'ตรายาง TESR']], TD.stamp.mode || 'text')}<div id="stText" style="display:${(TD.stamp.mode || 'text') === 'text' ? 'flex' : 'none'};flex-direction:column;gap:10px">${F.seg('stP', 'แบบสำเร็จ', [['อนุมัติ', 'อนุมัติ'], ['ตรวจแล้ว', 'ตรวจแล้ว'], ['ด่วน', 'ด่วน'], ['ลับ', 'ลับ'], ['ร่าง', 'ร่าง'], ['APPROVED', 'APPROVED'], ['CONFIDENTIAL', 'CONFIDENTIAL'], ['DRAFT', 'DRAFT']], TD.stamp.text)}${F.text('stT', 'หรือพิมพ์เอง', TD.stamp.text)}${F.color('stC', 'สี', TD.stamp.color)}</div><div id="stRub" style="display:${TD.stamp.mode === 'rubber' ? 'flex' : 'none'};flex-direction:column;gap:10px">${r.asset ? `<img src="${assets[r.asset].url}" style="max-height:150px;object-fit:contain;background:#fff;border-radius:8px;padding:6px">` : ''}${F.seg('rbShape', 'รูปแบบ', [['logo', 'โลโก้อย่างเดียว'], ['round', 'ตรากลม'], ['rect', 'สี่เหลี่ยม']], r.shape)}${F.seg('rbInk', 'สีหมึก', [['#c8102e', 'แดง'], ['#1a3fb8', 'น้ำเงิน'], ['#111111', 'ดำ'], ['#1f7a3a', 'เขียว']], r.ink)}<div id="rbTxt" style="display:${r.shape === 'logo' ? 'none' : 'flex'};flex-direction:column;gap:10px">${F.text('rbT1', 'ชื่อหลัก', r.t1)}${F.text('rbT2', 'ข้อความรอบตรา', r.t2)}</div>${F.text('rbMid', 'คำใต้ตรา (ไม่บังคับ)', r.mid, 'เช่น อนุมัติ')}${F.chk('rbDate', 'ใส่วันที่วันนี้', r.date)}<button class="btn primary" id="rbMake">${r.asset ? 'สร้างตรายางใหม่' : 'สร้างตรายาง'}</button></div><div class="note">แตะบนหน้าเพื่อประทับ แล้วย้าย/ย่อขยาย/ปรับความทึบได้</div></div>`; bind = () => { $('#stMode').addEventListener('input', () => { TD.stamp.mode = V('stMode'); $('#stText').style.display = TD.stamp.mode === 'text' ? 'flex' : 'none'; $('#stRub').style.display = TD.stamp.mode === 'rubber' ? 'flex' : 'none'; if (TD.stamp.mode === 'rubber' && !r.asset) buildRubberAsset(); }); ['rbShape', 'rbInk', 'rbT1', 'rbT2', 'rbMid', 'rbDate'].forEach(id => $('#' + id).addEventListener('input', () => { Object.assign(r, { shape: V('rbShape'), ink: V('rbInk'), t1: V('rbT1'), t2: V('rbT2'), mid: V('rbMid'), date: V('rbDate') }); $('#rbTxt').style.display = r.shape === 'logo' ? 'none' : 'flex'; })); $('#rbMake').onclick = buildRubberAsset; $('#stP').addEventListener('input', () => { TD.stamp.text = V('stP'); $('#stT').value = TD.stamp.text; }); $('#stT').addEventListener('input', () => TD.stamp.text = V('stT')); $('#stC').addEventListener('input', () => TD.stamp.color = V('stC')); }; break; }
      case 'stamp_old': html = head('ตราประทับ') + `<div class="opt">${F.seg('stP', 'แบบสำเร็จ', [['อนุมัติ', 'อนุมัติ'], ['ตรวจแล้ว', 'ตรวจแล้ว'], ['ด่วน', 'ด่วน'], ['ลับ', 'ลับ'], ['ร่าง', 'ร่าง'], ['APPROVED', 'APPROVED'], ['CONFIDENTIAL', 'CONFIDENTIAL'], ['DRAFT', 'DRAFT']], TD.stamp.text)}${F.text('stT', 'หรือพิมพ์เอง', TD.stamp.text)}${F.color('stC', 'สี', TD.stamp.color)}<div class="note">แตะบนหน้าเพื่อประทับ</div></div>`; bind = () => { $('#stP').addEventListener('input', () => { TD.stamp.text = V('stP'); $('#stT').value = TD.stamp.text; }); $('#stT').addEventListener('input', () => TD.stamp.text = V('stT')); $('#stC').addEventListener('input', () => TD.stamp.color = V('stC')); }; break;
...
/* ---------- TESR rubber stamp generator ---------- */
async function makeRubberStamp(o) {
  const S = 600, c = document.createElement('canvas'); c.width = c.height = S; const ctx = c.getContext('2d'); const ink = o.ink;
  const logo = await new Promise(r => { const im = new Image(); im.onload = () => r(im); im.onerror = () => r(null); im.src = 'logo.png'; });
  // 1) monochrome ink silhouette of the logo (dark pixels -> ink, white -> transparent)
  const lc = document.createElement('canvas'); const L = o.shape === 'logo' ? 520 : o.shape === 'round' ? 250 : 300; lc.width = lc.height = L; const lx = lc.getContext('2d');
  if (logo) { const sc = Math.min(L / logo.width, L / logo.height) * .92; lx.drawImage(logo, (L - logo.width * sc) / 2, (L - logo.height * sc) / 2, logo.width * sc, logo.height * sc); const d = lx.getImageData(0, 0, L, L), px = d.data; const n = parseInt(ink.slice(1), 16), R = n >> 16 & 255, G = n >> 8 & 255, B = n & 255; for (let i = 0; i < px.length; i += 4) { const lum = (.3 * px[i] + .59 * px[i + 1] + .11 * px[i + 2]) / 255; let a = px[i + 3] / 255 * clamp((1 - lum) * 1.6 - .15, 0, 1); px[i] = R; px[i + 1] = G; px[i + 2] = B; px[i + 3] = a * 255; } lx.putImageData(d, 0, 0); }
  ctx.strokeStyle = ctx.fillStyle = ink; ctx.lineCap = 'round';
  try { await document.fonts.load('700 30px Kanit'); } catch { }
  const cx = S / 2, cy = S / 2;
  if (o.shape === 'logo') { // logo only
    const extra = (o.mid ? 44 : 0) + (o.date ? 30 : 0); ctx.drawImage(lc, cx - L / 2, cy - L / 2 - extra / 2);
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; let ty = cy + L / 2 - extra / 2 + 6;
    if (o.mid) { ctx.font = '700 40px Kanit, sans-serif'; ctx.fillText(o.mid, cx, ty + 14); ty += 44; } if (o.date) { ctx.font = '600 24px Sarabun, sans-serif'; ctx.fillText(new Date().toLocaleDateString('th-TH', { day: '2-digit', month: 'short', year: 'numeric' }), cx, ty + 10); }
  } else if (o.shape === 'round') {
    ctx.lineWidth = 9; ctx.beginPath(); ctx.arc(cx, cy, 282, 0, Math.PI * 2); ctx.stroke(); ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(cx, cy, 266, 0, Math.PI * 2); ctx.stroke(); ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(cx, cy, 196, 0, Math.PI * 2); ctx.stroke();
    const ring = (txt, r, start, dir, size) => { ctx.font = `700 ${size}px Kanit, Sarabun, sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; const chars = [...txt]; const widths = chars.map(ch => ctx.measureText(ch).width + size * .18); const total = widths.reduce((a, b) => a + b, 0) / r; let ang = start - dir * total / 2; chars.forEach((ch, k) => { const w = widths[k] / r; ang += dir * w / 2; ctx.save(); ctx.translate(cx + r * Math.cos(ang), cy + r * Math.sin(ang)); ctx.rotate(ang + (dir > 0 ? Math.PI / 2 : -Math.PI / 2)); ctx.fillText(ch, 0, 0); ctx.restore(); ang += dir * w / 2; }); };
    ring(o.t1, 231, -Math.PI / 2, 1, 34); ring(o.t2, 231, Math.PI / 2, -1, 22);
    [-Math.PI, 0].forEach(a => { ctx.beginPath(); ctx.arc(cx + 231 * Math.cos(a), cy + 231 * Math.sin(a), 5, 0, Math.PI * 2); ctx.fill(); });
    ctx.drawImage(lc, cx - L / 2, cy - L / 2 - (o.mid || o.date ? 22 : 0));
    ctx.font = '700 26px Kanit, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; let ty = cy + 128;
    if (o.mid) { ctx.fillText(o.mid, cx, ty); ty += 30; } if (o.date) { ctx.font = '600 22px Sarabun, sans-serif'; ctx.fillText(new Date().toLocaleDateString('th-TH', { day: '2-digit', month: 'short', year: 'numeric' }), cx, ty); }
  } else { // rectangular
    const W = 560, H = 340, x0 = (S - W) / 2, y0 = (S - H) / 2; ctx.lineWidth = 9; ctx.strokeRect(x0, y0, W, H); ctx.lineWidth = 3; ctx.strokeRect(x0 + 14, y0 + 14, W - 28, H - 28);
    ctx.drawImage(lc, x0 + 28, y0 + 20, L, L);
    ctx.textAlign = 'left'; ctx.textBaseline = 'middle'; const tx = x0 + 40 + L, maxW = x0 + W - 34 - tx; let ty = y0 + 70;
    const fit = (txt, wgt, size, fam) => { let sz = size; ctx.font = `${wgt} ${sz}px ${fam}, sans-serif`; while (ctx.measureText(txt).width > maxW && sz > 10) { sz -= 1; ctx.font = `${wgt} ${sz}px ${fam}, sans-serif`; } ctx.fillText(txt, tx, ty); return sz; };
    ty += fit(o.t1, 700, 44, 'Kanit') * .9; fit(o.t2, 600, 17, 'Kanit'); ty += 48;
    if (o.mid) { ty += fit(o.mid, 700, 38, 'Kanit') * 1.1; } if (o.date) { fit('วันที่ ' + new Date().toLocaleDateString('th-TH', { day: '2-digit', month: 'short', year: 'numeric' }), 600, 24, 'Sarabun'); }
  }
  // 2) ink texture: erode random specks so it looks stamped
  const d = ctx.getImageData(0, 0, S, S), px = d.data; let seed = 7; const rnd = () => (seed = (seed * 16807) % 2147483647) / 2147483647;
  for (let y = 0; y < S; y++) for (let x = 0; x < S; x++) { const i = (y * S + x) * 4; if (!px[i + 3]) continue; const n = rnd(); const edge = (x < 40 || y < 40 || x > S - 40 || y > S - 40) ? .25 : 0; px[i + 3] *= n < .06 + edge ? .25 + rnd() * .5 : (.82 + rnd() * .18); }
  ctx.putImageData(d, 0, 0);
  // 3) slight tilt like a hand stamp
  const oc = document.createElement('canvas'); oc.width = oc.height = S; const ox = oc.getContext('2d'); ox.translate(S / 2, S / 2); ox.rotate((-4 + rnd() * 8) * Math.PI / 180); ox.drawImage(c, -S / 2, -S / 2);
  // trim transparent margin
  const id = ox.getImageData(0, 0, S, S).data; let a0 = S, b0 = S, a1 = 0, b1 = 0; for (let y = 0; y < S; y += 2) for (let x = 0; x < S; x += 2) if (id[(y * S + x) * 4 + 3] > 8) { if (x < a0) a0 = x; if (x > a1) a1 = x; if (y < b0) b0 = y; if (y > b1) b1 = y; }
  const tc = document.createElement('canvas'); tc.width = a1 - a0 + 8; tc.height = b1 - b0 + 8; tc.getContext('2d').drawImage(oc, a0 - 4, b0 - 4, tc.width, tc.height, 0, 0, tc.width, tc.height);
  return { id: uid(), bytes: await canvasBytes(tc, 'image/png'), type: 'image/png', w: tc.width, h: tc.height, url: tc.toDataURL() };
}
async function buildRubberAsset() { busy(true, 'กำลังสร้างตรายาง…'); try { const a = await makeRubberStamp(TD.rubber); assets[a.id] = a; TD.rubber.asset = a.id; TD.stamp.mode = 'rubber'; renderProps(); toast('แตะบนหน้าเพื่อประทับตรายาง TESR'); } finally { busy(false); } }
